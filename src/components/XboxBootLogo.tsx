...
interface XboxBootLogoProps {
  size?: number
}

export function XboxBootLogo({ size = 120 }: XboxBootLogoProps) {
  ...
  useEffect(() => {
    ...
    path.style.animation = 'xbox-boot-draw 2s ease-in-out forwards'
  }, [])

  return (
    <>
      <style>{`
        @keyframes xbox-boot-draw {
          to {
            stroke-dashoffset: 0;
          }
        }

        @keyframes xbox-boot-fade-fill {
          from {
            fill-opacity: 0;
          }
          to {
        ...
        .xbox-boot-logo {
          animation: xbox-boot-fade-fill 0.4s ease-in 1.8s forwards;
          fill-opacity: 0;
        }
      `}</style>
      <svg
